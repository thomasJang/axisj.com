...
처음에 시작할 땐 간단히 웹소켓으로 선수 정보를 전송하고 데이터베이스에 선수의 판정 결과를 저장하는 수준의 간단한 프로그램일 거로 생각했다. 그래서 작업기간 역시 간단하게 1주일 안에 끝낼 수 있을 것 같았다. 이런 생각을 할 수 있었던 건 어드민의 경우 기존에 개발되었던 장애인용 역도 경기 시스템을 재활용할 수 있다고 생각했기 때문이었다.

![경기시스템구성도](/post/weightlifting/img2.png?w=1000)

나는 간단하게 시스템구성도를 작성하고 작업에 착수했다. 작업 대부분은 백엔드 개발자들이 API와 웹소켓용 구독 메시지 포맷을 정하고 나면 디자이너에게 전달받은 화면을 연결하는 수준에서 작업을 마치려고 했다.
...
전광판에는 선수들의 도전 결과와 현재 경기중인 선수 정보를 한국에 확인할 수 있게 하였고, 한 경기에 여러 체급이 경기를 진행하는 경우를 대비하여 체급별 구분선을 추가하였다. 또한 우측 패널에는 현재시간과 세계기록, 동영상 출력 등의 기능을 두어 전광판에서 경기에 필요한 모든 정보를 표시할 수 있게 하였다.
